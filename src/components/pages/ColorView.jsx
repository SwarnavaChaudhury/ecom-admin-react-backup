...
export default function ColorView() {
    ...
    const searchRecord = (event) => {
        ...
        setNameFilter(event.target.search_clr.value);
    ...
    var checkedAll = () => {
        if (colors.length != checkedValue.length) {
            let data = [];
    ...
    let changeStatus = () => {
        ...
        if (checkedValue.length > 0) {
            axios.put(`http://localhost:8000/api/admin/color/change-status`, {
                id: checkedValue
            })
                .then((response) => {
                    if (response.data._status == true) {
    ...
    let deleleRecords = () => {
        ...
        if (checkedValue.length > 0) {
            if (confirm('Confirm delete those records?')) {
                axios.put(`http://localhost:8000/api/admin/color/delete`, {
                    id: checkedValue
                })
                    .then((response) => {
                        if (response.data._status == true) {
        ...
        <>
            ...
            <div className='max-w-[1320px] mx-auto gap-3 mb-50 mt-10'>
                ...
                <div className={`
                                    ...
                                    ${searchfilter ? "hidden" : ""}
                                `}>
                    <form className='flex gap-2' onSubmit={searchRecord}>
                        <input type="text" name="search_clr" id="search_clr" className='h-[40px] w-100 rounded-lg' autoComplete='off' />
                        ...
                        {/* <div className='w-full py-3 flex justify-center items-center gap-2'>
                            {
                                colors.length > 0
                                    ?
                                    Array.from({ length: colorPagination.total_pages }, (_, index) => {
                                        return (
                                            <button
